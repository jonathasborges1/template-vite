...
export default class AxiosHttpService implements IHttpService {
  ...
  // É importante alterar no appConfig o valor do appConfig.api e appConfig.url
  constructor() {
  ...
  private async makeRequest<T = any>(method: string, path: string, body?: any, params?: {}): Promise<T> {
    try {
      ...
      throw new AppError(`Não foi possível realizar a operação: ${axiosError.message}`, "error");
    }
  }
